...
class Config:
    def __init__(self, config_dict):
        # 使用 dict 的每个键值对初始化实例的属性
        for key, value in config_dict.items():
            setattr(self, key, value)

# 直接硬编码 R50-ViT-B_16 配置
CONFIG_R50_B16 = {
    'patches': {'grid': (16, 16)},
    'hidden_size': 768,
    'transformer': {
    ...
    'n_skip': 3,
    'activation': 'softmax',
    'resnet': {'num_layers': (3, 4, 9), 'width_factor': 1},
}

# 将 CONFIG_R50_B16 转换为 Config 对象
config_obj = Config(CONFIG_R50_B16)
...
class DA_Transformer(nn.Module):
    def __init__(self, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        self.classifier = CONFIG_R50_B16['classifier']
        self.transformer = Transformer(CONFIG_R50_B16, img_size, vis)
        self.decoder = DecoderCup(CONFIG_R50_B16)
        self.segmentation_head = SegmentationHead(
            in_channels=CONFIG_R50_B16['decoder_channels'][-1],
            out_channels=CONFIG_R50_B16['n_classes'],
            kernel_size=3,
        )
        self.config = CONFIG_R50_B16
